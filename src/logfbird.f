ccccccccc Fortran subroutine: logfbird cccccccccccccccccc

c For computing the logarithm of
c the unnormalized density function
c for slice sampling for GLMM

c Last changed: 27 SEP 2010

      subroutine logfbird(nu,j,CTy,Cmat,Cnurest,sigsqnu,ncC,n,logf,
 ...
 10      continue  
         ...
         do 20 i = 1,ubkid(1)
            logf = logf - log(1+exp(nu + Cnurest(i)))
 20      continue
 
         do 30 i = lbkid(2),n
            logf = logf - log(1+exp(Cnurest(i)))
 30      continue   
...
cccccccccc End of subroutine logfbird cccccccccc
