ccccccccc Fortran subroutine: slbsprd.f cccccccccccccccccc
...
c Last changed:  21 JAN 2012
      ...
      subroutine slbsprd(y,Cmat,CTy,nobs,ncC,ncX,ngps,nreb,rbei,ngb,
...
c     Update the 
 ...
 150           continue
            ...
            call logfbird(nucurrj,j,CTy,Cmat,Cnurest,sigsqnu,ncC,
 ...
 160        call logfbird(Lnuj,j,CTy,Cmat,Cnurest,sigsqnu,ncC,
     +           nobs,logfval,ubkid,lbkid,ngps,ncX)
            if(a.lt.logfval)then
               Lnuj = Lnuj - w
               goto 160
            endif
 170        call logfbird(Unuj,j,CTy,Cmat,Cnurest,sigsqnu,ncC,
     +           nobs,logfval,ubkid,lbkid,ngps,ncX)
            if(a.lt.logfval)then
               Unuj = Unuj + w
               goto 170
 ...
 180        call urand(1,uvar)
            ...
            call logfbird(nujnew,j,CTy,Cmat,Cnurest,sigsqnu,ncC,
     +           nobs,logfval,ubkid,lbkid,ngps,ncX)
            if (a.lt.logfval)then
               ind = (ig-1)*ncC + j
               nu(ind) = nujnew
...
ccccccccccccccccccccccc-slbsprd.f-ccccccccccccccccccc
